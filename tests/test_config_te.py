...
def test_te_api_key_por_defecto_vacia(monkeypatch, tmp_path):
    _reset_cache(monkeypatch, tmp_path / 'config.json')
    assert config.get_te_api_key() == ''


def test_set_te_api_key_persiste_y_recarga(monkeypatch, tmp_path):
    cfg_path = tmp_path / 'config.json'
    _reset_cache(monkeypatch, cfg_path)
    monkeypatch.setattr(config, 'BASE_DATA', str(tmp_path))

    config.set_te_api_key('  abc123  ')
    assert config.get_te_api_key() == 'abc123'   # se recorta

    monkeypatch.setattr(config, '_APP_CONFIG_CACHE', {})
    assert config.get_te_api_key() == ''
    config.load_app_config()
    assert config.get_te_api_key() == 'abc123'
